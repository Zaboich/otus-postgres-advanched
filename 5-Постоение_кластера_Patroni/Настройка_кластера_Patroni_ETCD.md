...
На этом этапе поочередно запускаем сервис с 1-й по 3-ю ноду с помощью команды: 
```
...
Важное примечание на полях: на astra linux встречали удаление директории  `/var/run/postgresql` для сокетов, если ставить Postgres из оф. репозитория. Это можно поправить, если изменить сервис Postgres и добавить 
```
...
```
А теперь активируем виртуальную среду: 
```
source /opt/patroni/bin/activate
```
## Настройка Patroni
...
Проверяем список узлов в кластере: 
```
patronictl list
```
